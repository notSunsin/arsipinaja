<x-app-layout>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex items-center space-x-1">
                                                ...
                                                @if (isset($showActionButtons) && $showActionButtons)
                                                    ...
                                                    <button
                                                        onclick="confirmEditArchive({{ $archive->id }}, '{{ addslashes($archive->index_number) }}', '{{ addslashes($archive->description) }}', {{ (int) $archive->created_by }}, '{{ addslashes($archive->createdByUser->name ?? 'User') }}')"
                                                        class="text-green-600 hover:text-green-800 hover:bg-green-50 p-1.5 rounded transition-colors"
                                                        title="Edit">
                                                        <i class="fas fa-edit text-sm"></i>
                                                    </button>
                                                    <button
                                                        onclick="confirmDeleteArchive({{ $archive->id }}, '{{ $archive->index_number }}', '{{ $archive->description }}', {{ (int) $archive->created_by }}, '{{ $archive->createdByUser->name ?? 'User' }}')"
    ...
    @push('scripts')
        ...
        <script>
            ...
            const isAdmin = {{ Auth::user()->role_type === 'admin' ? 'true' : 'false' }};

            // Delete confirmation with SweetAlert
            function confirmDeleteArchive(archiveId, indexNumber, description, createdBy, createdByName) {
                // Intern mencoba hapus arsip milik user lain
                if (!isAdmin && currentUserId !== createdBy) {
                    Swal.fire({
                        title: 'Tidak Dapat Menghapus Arsip',
                        html: `
                            <div class="text-left">
                                <p class="mb-3">Anda tidak memiliki izin untuk menghapus arsip ini.</p>
                                <div class="bg-gray-50 p-3 rounded-lg">
                                    <p class="font-semibold text-gray-800">Nomor Arsip: ${indexNumber}</p>
                                    <p class="text-gray-600 text-sm">${description}</p>
                                </div>
                                <p class="text-orange-600 text-sm mt-3">
                                    <i class="fas fa-info-circle mr-1"></i>
                                    Arsip ini dibuat oleh <strong>${createdByName}</strong>. Anda hanya dapat menghapus arsip yang Anda buat sendiri.
                                </p>
                            </div>
                        `,
                        icon: 'error',
                        confirmButtonColor: '#f59e0b',
                        confirmButtonText: '<i class="fas fa-times mr-2"></i>Mengerti',
                    });
                    return;
                }
            ...
            // Edit confirmation with SweetAlert
            function confirmEditArchive(archiveId, indexNumber, description, createdBy, createdByName) {
                if (!isAdmin && currentUserId !== createdBy) {
                    Swal.fire({
                        title: 'Tidak Dapat Mengedit Arsip',
                        html: `
                            <div class="text-left">
                                <p class="mb-3">Anda tidak memiliki izin untuk mengedit arsip ini.</p>
                                <div class="bg-gray-50 p-3 rounded-lg">
                                    <p class="font-semibold text-gray-800">Nomor Arsip: ${indexNumber}</p>
                                    <p class="text-gray-600 text-sm">${description}</p>
                                </div>
                                <p class="text-orange-600 text-sm mt-3">
                                    <i class="fas fa-info-circle mr-1"></i>
                                    Arsip ini dibuat oleh <strong>${createdByName}</strong>. Anda hanya dapat mengedit arsip yang Anda buat sendiri.
                                </p>
                            </div>
                        `,
                        icon: 'error',
                ...
                window.location.href = `/intern/archives/${archiveId}/edit`;
            }
        </script>
    @endpush
