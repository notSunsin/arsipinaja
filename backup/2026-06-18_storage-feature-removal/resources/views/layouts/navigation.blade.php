    ...
    <div class="fixed inset-y-0 left-0 z-40 w-64 bg-white shadow-lg transform lg:translate-x-0 lg:static lg:inset-0 transition-transform duration-200 ease-out sidebar-stable"
                ...
                <div class="text-white">
                    ...
                    <p class="text-sm text-white/80 font-medium">
                        @if (auth()->check() && auth()->user()->role_type === 'admin')
                            Admin Portal
                        @elseif(auth()->check() && auth()->user()->role_type === 'intern')
                            Portal Magang
                        @else
                            Sistem Arsip Pintar
                        @endif
        ...
        <nav class="flex-1 px-4 py-4 space-y-1 overflow-y-auto nav-transition">
            ...
            <a href="{{ auth()->check() ? route(auth()->user()->getDashboardRoute()) : '#' }}" @click="closeSidebar()"
                ...
                {{ request()->routeIs('admin.dashboard', 'intern.dashboard') ? 'bg-blue-50 text-blue-700 border-r-4 border-blue-700' : 'text-gray-600 hover:bg-blue-50 hover:text-blue-700 hover:translate-x-1' }}">
            ...
            <div class="space-y-1 submenu-container">
                <button @click="toggleArchiveSubmenu()"
                    ...
                    {{ request()->routeIs('*.archives.*') ? 'bg-blue-50 text-blue-700 border-r-4 border-blue-700' : 'text-gray-600 hover:bg-blue-50 hover:text-blue-700 hover:translate-x-1' }}">
                    <div class="flex items-center">
                ...
                <div x-show="archiveSubmenuOpen" x-transition:enter="transition ease-out duration-150"
                    ...
                    <a href="{{ auth()->check() && auth()->user()->role_type === 'admin' ? route('admin.archives.create') : route('intern.archives.create') }}"
                        @click="closeSidebar()"
                    ...
                    <a href="{{ auth()->check() && auth()->user()->role_type === 'admin' ? route('admin.archives.index') : route('intern.archives.index') }}"
                        @click="closeSidebar()"
                        class="flex items-center px-4 py-2 text-sm rounded-lg transition-all duration-200
                    ...
                    <!-- Status-based Archives - Admin Only -->
                    @if (auth()->check() && auth()->user()->role_type === 'admin')
                        <a href="{{ route('admin.archives.aktif') }}"
                            @click="closeSidebar()"
                            class="flex items-center px-4 py-2 text-sm rounded-lg transition-all duration-200
                        ...
                        <a href="{{ route('admin.archives.inaktif') }}"
                            @click="closeSidebar()"
                            class="flex items-center px-4 py-2 text-sm rounded-lg transition-all duration-200
                        ...
                        <a href="{{ route('admin.archives.musnah') }}"
                            @click="closeSidebar()"
                            class="flex items-center px-4 py-2 text-sm rounded-lg transition-all duration-200
                        ...
                        <a href="{{ route('admin.archives.permanen') }}"
                            @click="closeSidebar()"
                            class="flex items-center px-4 py-2 text-sm rounded-lg transition-all duration-200
                            ...
                            <i class="fas fa-archive mr-3 text-sm w-4 transition-colors duration-200"></i>
                            Arsip Permanen
                        </a>
            ...
            <div class="space-y-1 submenu-container">
                ...
                <div x-show="storageSubmenuOpen" x-transition:enter="transition ease-out duration-150"
                    ...
                    <a href="{{ auth()->check() && auth()->user()->role_type === 'admin' ? route('admin.storage.index') : route('intern.storage.index') }}"
                        @click="closeSidebar()"
                        class="flex items-center px-4 py-2 text-sm rounded-lg transition-all duration-200
                    ...
                    <a href="{{ auth()->check() && auth()->user()->role_type === 'admin' ? route('admin.storage-management.index') : route('intern.storage-management.index') }}"
                        @click="closeSidebar()"
                        class="flex items-center px-4 py-2 text-sm rounded-lg transition-all duration-200
