<x-app-layout>
    ...
    <script>
        ...
        function confirmEditArchive() {
            const currentUserId = {{ (int) Auth::id() }};
            const createdBy = {{ (int) $archive->created_by }};
                    ...
                    html: `
                        <div class="text-left">
                            ...
                            <div class="bg-gray-50 p-3 rounded-lg">
                                <p class="font-semibold text-gray-800">Nomor Arsip: {{ $archive->index_number }}</p>
                                <p class="text-gray-600 text-sm">{{ $archive->description }}</p>
                            </div>
                            <p class="text-orange-600 text-sm mt-3">
                                <i class="fas fa-info-circle mr-1"></i>
                                Arsip ini dibuat oleh <strong>{{ $archive->createdByUser->name ?? 'User lain' }}</strong>. Anda hanya dapat mengedit arsip yang Anda buat sendiri.
                            </p>
                        </div>
                    `,
                    icon: 'error',
            ...
            window.location.href = '{{ route('intern.archives.edit', $archive) }}';
        }
    </script>
    @endpush
</x-app-layout>
