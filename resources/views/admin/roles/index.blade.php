                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200">
                    <div class="p-6 border-b border-gray-200">
                        <div class="flex items-center justify-between">
                            ...
                            <span
                                class="text-sm bg-purple-100 text-purple-700 px-3 py-1 rounded-full">{{ $roles->count() }}
                                roles</span>
                        </div>
                    </div>
                    <div class="p-6">
                        <div class="space-y-3">
                            @foreach ($roles as $role)
                                ...
                                <div
                                    class="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:shadow-md transition-all">
                                    <div class="flex items-center space-x-3">
                                        <div
                                            class="w-10 h-10 bg-{{ $roleConfig['color'] }}-100 rounded-lg flex items-center justify-center">
                                            <i
                                                class="{{ $roleConfig['icon'] }} text-{{ $roleConfig['color'] }}-600"></i>
                                        </div>
                                        <div>
                                            <h4 class="font-semibold text-gray-900 capitalize">{{ $role->name }}</h4>
                                            <div class="flex items-center space-x-3 text-xs text-gray-500">
                                                <span><i class="fas fa-users mr-1"></i>{{ $role->users_count }}
                                                    users</span>
                                                <span><i class="fas fa-key mr-1"></i>{{ $role->permissions_count }}
                                                    permissions</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="flex items-center space-x-2">
                                        ...
                                        @if (!in_array($role->name, ['admin', 'intern']))
                                ...
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4">
                                        @if ($user->roles->count() > 0)
                                            <input type="checkbox"
                                                class="user-role-checkbox w-4 h-4 text-orange-600 bg-gray-100 border-gray-300 rounded focus:ring-orange-500 focus:ring-2"
                                        ...
                                        @forelse($user->roles as $role)
                                            @php
                                                ...
                                                $roleIcons = match($role->name) {
                                                    'admin' => 'fas fa-crown',
                                                    'intern' => 'fas fa-graduation-cap',
                                                    default => 'fas fa-user-cog'
                                                };
                                            @endphp
                                            <span
                                                class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium {{ $roleColors }} mr-1 mb-1">
                                                <i class="{{ $roleIcons }} mr-1.5 text-xs"></i>
                                                {{ ucfirst($role->name) }}
                                            </span>
                                        @empty
    ...
    @push('scripts')
        <script>
            ...
            function showCenterNotification(message, type = 'success', duration = 3000) {
                ...
                const iconClass = type === 'success' ? 'fa-check-circle' : (type === 'error' ? 'fa-exclamation-circle' :
                    'fa-info-circle');
            ...
            function deleteRole(id, name) {
                window.showDeleteConfirm(
                    `Apakah Anda yakin ingin menghapus role "${name}"? Tindakan ini tidak dapat dibatalkan dan akan menghapus semua permissions terkait.`
                ).then((result) => {
                    if (result.isConfirmed) {
                        // Show loading
                        window.showMessage.info('Menghapus role...');
            ...
            if (assignRoleForm) {
                ...
                if (!userId || !roleId) {
                    window.showValidationError('Mohon pilih user dan role terlebih dahulu!');
                    return;
                }
                ...
                window.showUpdateConfirm(`Assign role "${roleName.split('(')[0].trim()}" ke user "${userName.split('(')[0].trim()}"?`)
                    .then((result) => {
                        if (result.isConfirmed) {
                            // Show loading
                            ...
                            .then(data => {
                                if (data.success) {
                                    window.showMessage.success(`✅ Role "${roleName.split('(')[0].trim()}" berhasil di-assign ke "${userName.split('(')[0].trim()}"`);
                                    this.reset();
                            ...
                            .catch(error => {
                                console.error('Error:', error);
                                window.showMessage.error('❌ Terjadi kesalahan saat assign role');
                            })
                            .finally(() => {
            ...
            function confirmRemoveUserRoles(userId, userName) {
                ...
                if (selectedRoles.length === 0) {
                    window.showMessage.error('❌ Pilih minimal satu role untuk dihapus');
                    return;
                }
            ...
            function executeRemoveUserRoles(userId, userName, roleIds) {
                window.showDeleteConfirm(`Apakah Anda yakin ingin menghapus role yang dipilih dari user "${userName}"?`)
                    .then((result) => {
                        if (result.isConfirmed) {
                            fetch('/admin/roles/remove-user-roles', {
                                ...
                                .then(data => {
                                    if (data.success) {
                                        window.showMessage.success(`✅ ${data.message || `Role berhasil dihapus dari "${userName}"`}`);
                                        setTimeout(() => location.reload(), 2000);
                                    } else {
                                        window.showMessage.error(`❌ ${data.message || 'Gagal menghapus role'}`);
                                    }
                                })
                                .catch(error => {
                                    console.error('Error:', error);
                                    window.showMessage.error('❌ Terjadi kesalahan saat menghapus role');
                                });
                        }
                    });
            }
            ...
            function deleteUser(id, name) {
                window.showDeleteConfirm(
                    ...
                    if (result.isConfirmed) {
                        // Show loading
                        window.showMessage.info('Menghapus user...');
            ...
            function executeBulkRemoveRoles(userIds) {
                fetch(`/admin/roles/bulk-remove-users`, {
                    ...
                    .then(response => response.json())
                    .then(data => {
                        showCenterNotification(data.message || `Role berhasil dihapus dari ${userIds.length} user!`,
                            'success');
                        setTimeout(() => location.reload(), 2000);
                    })
                    .catch(error => {
